...
public class CharacterController2D : MonoBehaviour {
	...
	void Update()
	{
        ...
        if (!isGrounded && Input.GetButtonDown("Jump") && canDoubleJump) // If grounded AND jump button pressed, then allow the player to jump
        {
            Jump(400f);
            canDoubleJump = false;
        }
        ...
        Vector2 shootDirection = new Vector2(Input.GetAxisRaw("Shoot_Horizontal"), Input.GetAxisRaw("Shoot_Vertical"));
        if(Time.time >= nextShootTime && shootDirection != Vector2.zero)
        {
...
void Shoot(Vector2 direction)
    {
        nextShootTime = Time.time + timeBetweenShooting;
        if (bullet)
        {
            GameObject newBullet = Instantiate(bullet) as GameObject;
            newBullet.transform.position = transform.position + new Vector3(0, -0.3f, 0);
            if (!newBullet.GetComponent<Rigidbody2D>())
            {
                newBullet.AddComponent<Rigidbody2D>();
            }
            Vector2 powerV = direction;
            newBullet.GetComponent<Rigidbody2D>().AddForce(powerV * power, ForceMode2D.Impulse);
            if (shootSFX)
            {
    ...
    void Jump(float jumpForce)
    {
        ...
        _vy = 0f;
        // add a force in the up direction
        _rigidbody.AddForce(new Vector2(0, y: jumpForce));
        // play the jump sound
        PlaySound(jumpSFX);
    }
}
